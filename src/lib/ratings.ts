...
export async function getAllRatingSummaries(): Promise<Record<string, RatingSummary>> {
  await ensureTable();
  const { rows } = await sql<{ course_id: string; average: string | null; count: string }>`
...
export async function getMyRatings(visitorId: string): Promise<Record<string, number>> {
  if (!visitorId) return {};
  await ensureTable();
  const { rows } = await sql<{ course_id: string; rating: number }>`
...
export async function upsertRating(
  ...
  `;
  const { rows } = await sql<{ average: string | null; count: string }>`
